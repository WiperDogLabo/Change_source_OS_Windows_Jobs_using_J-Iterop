	...
	def start(sccontext, senderList) {
		try{
			...
			if(strMorType.equals(ResourceConstants.MONITORINGTYPE_OS)){
				...
				if(osInfo["domain"] != null && osInfo["domain"].trim() != "" && osInfo["host"] != null 
				   && osInfo["host"].trim() != ""  && osInfo["user"] != null  && osInfo["user"].trim() != "" ) {
					def wmiAccessor = new WMIAccessor(osInfo["domain"],osInfo["host"],osInfo["user"],osInfo["pass"])
					binding.setVariable("wmiAcc", wmiAccessor)
				} else {
				    println "[DefaultJobCaller] - OS information params must be not null or empty for following fields: domain, host, user"
				}
			}
			if(strHostId == null) {
